 ...
 * Name: sendFile
 * Preconditions:
 * @param socket - socket with connection to otp_enc_d
...
int sendFile(int* socket, char *fileName) {
    ...
    // Make sure otp_enc_d received the file size
...
int receiveMessage(int *socket) {
    ...
    // Get message from otp_enc_d
    if ((numBytes = recv(*socket, inMessage, MAXBUFFERSIZE, 0)) == -1) {
        error("failure receiving from otp_enc_d", false);
        return -1;
    } else {
        inMessage[numBytes] = '\0';
    }

    // If connection is not with otp_enc_d
...
void validateKey(char* key, int numPlaintextChars) {
...
int validatePlaintext(char* filename) {
    ...
    if (numChars == -1) {
        // File could not be opened
        error("unable to open plaintext file", true);
    } else if (numChars == -2) {
        // Invalid character
        error("plaintext file contains bad characters", true);
    }

    return numChars;
}

// Get cyphertext from otp_enc_d and print to stdout
// https://stackoverflow.com/questions/11952898/c-send-and-receive-file
